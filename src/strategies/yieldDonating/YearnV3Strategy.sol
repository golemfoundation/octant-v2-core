...
contract YearnV3Strategy is BaseHealthCheck {
     ...
     * @dev Validates asset matches Yearn vault's asset. Allowance is granted per-deposit
     *      in `_deployFunds` to avoid leaving a standing claim against idle balances.
     ...
     * @custom:security Approves exactly `_amount`; Yearn's `deposit` pulls the full amount
     *                  so the allowance settles back to zero, leaving no standing claim
     *                  against the strategy's idle balance.
     */
