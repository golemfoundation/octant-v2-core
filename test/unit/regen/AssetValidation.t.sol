...
contract RegenStakerBaseAssetValidationTest is Setup {
    ...
    function setUp() public {
        ...
        // Deploy calculator
        calculator = _deployCalculator(admin, IAddressSet(address(0)), IAddressSet(address(0)), AccessMode.NONE);
        ...
        regenStaker = _deployRegenStaker(
            RegenCfg({
                ...
                stakerAllowset: IAddressSet(address(allowset)),
                stakerBlockset: IAddressSet(address(0)),
                stakerAccessMode: AccessMode.NONE,
                allocationMechanismAllowset: IAddressSet(address(allocationAllowset))
            })
        );
