...
contract AccountingTest is Setup {
    ...
    function test_airdropDoesNotIncreasePPS_reportRecordsIt(
        ...
        assertEq(asset.balanceOf(dragonRouter), toAirdrop, "!dragonRouter");
    ...
    /**
     * @notice Test burn conversion accounting fix for dragonRouter loss protection
    ...
    function test_burnConversion_correctAccounting() public {
        ...
        // Setup: totalShares == totalAssets == 100, dragonRouter has 20 shares, loss = 25
        mintAndDepositIntoStrategy(strategy, user, 100e18);
        vm.prank(keeper);
        strategy.report();

        vm.prank(user);
        strategy.transfer(dragonRouter, 20e18);
        ...
        // Report the loss (triggers the fixed _handleDragonRouterLossProtection)
        vm.prank(keeper);
        ...
        // 1. All dragonRouter shares burned
        assertEq(strategy.balanceOf(dragonRouter), 0, "All dragonRouter shares should be burned");
    ...
    function test_burnConversion_variousScenarios() public {
        ...
        // Scenario 1: Loss fully covered by dragonRouter shares
        mintAndDepositIntoStrategy(strategy, user, 100e18);
        vm.prank(keeper);
        strategy.report();

        vm.prank(user);
        strategy.transfer(dragonRouter, 30e18);
        ...
        assertEq(strategy.balanceOf(dragonRouter), 10e18, "10 dragonRouter shares should remain");
        ...
        // Scenario 2: Minimal dragonRouter shares, large loss
        vm.startPrank(user);
        strategy.transfer(dragonRouter, 9e18); // DragonRouter now has 19 total shares
        vm.stopPrank();
        ...
        // Should burn all 19 dragonRouter shares, covering 19 tokens
        assertEq(strategy.balanceOf(dragonRouter), 0, "All dragonRouter shares should be burned");
    ...
    function test_burnConversion_burningDisabled() public {
        ...
        // Transfer shares to dragonRouter
        vm.prank(user);
        strategy.transfer(dragonRouter, 30e18);
        ...
        uint256 dragonRouterBalanceBefore = strategy.balanceOf(dragonRouter);
        ...
        assertEq(
            ...
            dragonRouterBalanceBefore,
            "DragonRouter balance should remain unchanged when burning disabled"
        );
    ...
    function test_burnConversion_rateConsistency() public {
        ...
        // Transfer 20 shares to dragonRouter
        vm.prank(user);
        strategy.transfer(dragonRouter, 20e18);
